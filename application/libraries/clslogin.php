    ...
    class clsLogin
    {
        ...
        */
            function login($user = "", $clave = "")
            {
                ...
                $sql = "SELECT usu_cedula, usu_nombre, usu_apellido, usu_estado
                        FROM tbl_usuario
                        WHERE usu_usuario=? AND usu_contrasena=? AND usu_estado=? AND usu_tipo = ?";
                
                $salt = '6&KTTmxa%Tej|y6uH%OhSrK@caXbNNo%I23tQmJ20Sid';
                $enc_pass = sha1(md5($salt.$clave));

                $query = $CI->db->query($sql, array($user, $enc_pass, 'A', 'S'));
